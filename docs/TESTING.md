...
summaries, and the popup's honest capability state. They do not test phishing
detection or submit credentials.
...
Use that hosted address in stage-end acceptance instructions. The fixtures use
no form submission, and Nginx accepts only GET and HEAD under `/fixtures/`.
Deploy the current fixture set with `pnpm deploy:test-download` before
requesting manual acceptance.
